...
Welcome. I love to bike; and since bicycling is a healthy and sustainable alternative to carbon-expensive modes of transit, I'm stubbornly curious about urban bicycle networks. Many systems now publish de-identified data in some form or another and I created the `cycleR` package to simplify cleaning processes from raw system data.

I hope this package is useful for you in some way, but bear in mind that I make a few assumptions about the incoming data you're working with:

1.  The data are observed at "ride-level" -- meaning that each row represents a unique start-end trip.

2.  The rideshare system is organized primarily into bikeshare stations, where each ride typically starts and ends at a known bike dock or station.

The general sequence of data flow is as follows, but this may change as I improve the package:

1.  Import the raw ride data

2.  Import the station data

3.  Join station and ride data

4.  Transform to Origin-Destination format
...
We could keep the data like this, but let's transform to origin-destination summary format. My use "Origin-Destination" comes from the `stplanr` [package from ROpenSci](https://docs.ropensci.org/stplanr/articles/stplanr-od.html). Our data in `classic` is observed at the ride-level, meaning that each line represents one distinct bikeshare ride from point A to B. Origin-Destination (OD) format summarizes this data such that one entry represents the number of rides in a given time period between two given stations.

OD format has a number of advantages compared to ride-level data. First, it compresses the data, which makes it easier and faster to perform analysis. Second, having an OD format allows us to interface with other existing packages, such as `stplanr`, and analyze node-to-node travel patterns.
...
We'll use `transform_od()` to rework our data into origin-destination format, long form, where each line represents a unique flow-time summary. The function reads `origin_datetime` and `destination_datetime` in POSIXct format, and requires the two start and end station number variables in the order `c("start_variable", "end_variable")` . The `granularity` argument is one of "day", "month", or "year"; this determines how to group the rides at each start-end station combination. Compare the two results of "day" and "month":

```{r}
od <- classic %>%
  transform_od(origin_datetime = startdt,
               destination_datetime = enddt,
               station_vars = c("start_id_proj", "end_id_proj"),
               member_var = member,
               granularity = "day")
od
```

Once in OD long form, we can perform flow analysis and create desire lines as in the `stplanr` package. Happy cycling!
